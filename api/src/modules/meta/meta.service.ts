...
export class MetaService {
  ...
  async create(
    tenantId: string,
    agenteId: string,
    dto: CreateMetaPublicacionDto,
  ) {
    if (dto.propiedad_id) {
      const prop = await this.prisma.propiedad.findFirst({
        where: { id: dto.propiedad_id, tenant_id: tenantId },
      });
  ...
  async update(tenantId: string, id: string, dto: UpdateMetaPublicacionDto) {
    ...
    if (pub.estado !== 'BORRADOR')
      throw new BadRequestException(
        'Solo se pueden editar publicaciones en estado BORRADOR',
      );
  ...
  async delete(tenantId: string, id: string) {
    ...
    if (pub.estado === 'PUBLICADA')
      throw new BadRequestException(
        'No se puede eliminar una publicación ya publicada',
      );
  ...
  async previewTexto(
    tenantId: string,
    propiedadId: string,
  ): Promise<{ mensaje: string; imagen_url: string | null }> {
    const prop = await this.prisma.propiedad.findFirst({
    ...
    if (prop.departamento)
      lines.push(
        `Zona: ${prop.departamento}${prop.municipio ? ', ' + prop.municipio : ''}`,
      );
    lines.push('');
    if (prop.precio_venta)
      lines.push(
        `💰 Venta: ${prop.moneda} ${Number(prop.precio_venta).toLocaleString('es-GT')}`,
      );
    if (prop.precio_renta)
      lines.push(
        `🔑 Renta: ${prop.moneda} ${Number(prop.precio_renta).toLocaleString('es-GT')}`,
      );
    if (prop.habitaciones) lines.push(`🛏 ${prop.habitaciones} habitaciones`);
    if (prop.banos) lines.push(`🚿 ${prop.banos} baños`);
    if (prop.area_terreno_m2)
      lines.push(
        `📐 Terreno: ${Number(prop.area_terreno_m2).toLocaleString()} m²`,
      );
    if (prop.area_construccion_m2)
      lines.push(
        `🏗 Construcción: ${Number(prop.area_construccion_m2).toLocaleString()} m²`,
      );
    lines.push('');
    lines.push(
  ...
  async publicar(tenantId: string, id: string) {
    ...
    if (!['BORRADOR', 'FALLIDA'].includes(pub.estado)) {
      throw new BadRequestException(
        'Solo se pueden publicar posts en estado BORRADOR o FALLIDA',
      );
    }
  ...
  async programar(tenantId: string, id: string, programadoPara: Date) {
    const pub = await this.get(tenantId, id);
    if (pub.estado !== 'BORRADOR')
      throw new BadRequestException('Solo borradores pueden programarse');

    const minTime = new Date(Date.now() + 10 * 60 * 1000);
    if (programadoPara < minTime)
      throw new BadRequestException(
        'La fecha debe ser al menos 10 minutos en el futuro',
      );

    const delay = programadoPara.getTime() - Date.now();
    await this.queue.add(
      'publish',
      { publicacionId: id, tenantId },
  ...
  async ejecutarPublicacion(id: string, tenantId: string) {
    ...
    try {
      ...
      if (
        (plataforma === 'FACEBOOK' || plataforma === 'AMBAS') &&
        fbConfigured
      ) {
        fbPostId = await this.publishFacebook(
          creds,
          pub.mensaje,
          pub.imagen_url,
        );
      ...
      if (
        ...
        igPostId = await this.publishInstagram(
          creds,
          pub.mensaje,
          pub.imagen_url,
        );
      ...
      if (!fbPostId && !igPostId) {
        throw new Error(
          !fbConfigured
            ? 'Credenciales de Meta no configuradas para este tenant'
      ...
      await this.prisma.metaPublicacion.update({
        where: { id },
        data: { estado: 'FALLIDA', error_msg: error },
      });
