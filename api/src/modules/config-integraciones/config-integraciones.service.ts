...
export class ConfigIntegracionesService {
  ...
  async findOrCreate(tenantId: string) {
    let row = await this.prisma.configIntegraciones.findUnique({ where: { tenant_id: tenantId } });
    if (!row) {
      row = await this.prisma.configIntegraciones.create({ data: { tenant_id: tenantId } });
    }
  ...
  async getCredentials(tenantId: string) {
    const row = await this.prisma.configIntegraciones.findUnique({ where: { tenant_id: tenantId } });
    ...
    return {
      ...
      whatsapp_phone_number_id:  plain(row?.whatsapp_phone_number_id,'WHATSAPP_PHONE_NUMBER_ID'),
      ...
      docusign_integration_key:  plain(row?.docusign_integration_key,'DOCUSIGN_INTEGRATION_KEY'),
      docusign_account_id:       plain(row?.docusign_account_id,     'DOCUSIGN_ACCOUNT_ID'),
      docusign_user_id:          plain(row?.docusign_user_id,        'DOCUSIGN_USER_ID'),
      docusign_rsa_private_key:  dec(row?.docusign_rsa_private_key,  'DOCUSIGN_RSA_PRIVATE_KEY'),
