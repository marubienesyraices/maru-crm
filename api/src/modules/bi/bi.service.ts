...
const BADGES_DEF = [
  {
    ...
    emoji: '🏆',
    label: 'Top Ventas',
    desc: 'Mayor número de cierres en el período',
  },
  {
    id: 'top_comision',
    emoji: '💰',
    label: 'Top Comisión',
    desc: 'Mayor comisión generada',
  },
  {
    id: 'mas_activo',
    emoji: '⚡',
    label: 'Más Activo',
    desc: 'Más interacciones registradas',
  },
  {
    id: 'tour_master',
    emoji: '🏠',
    label: 'Tour Master',
    desc: 'Más visitas realizadas',
  },
  {
    id: 'elite',
    emoji: '🎯',
    label: 'Élite',
    desc: 'Conversión ≥ 70 % con 2+ cierres',
  },
  {
    id: 'cerrador',
    emoji: '⭐',
    label: 'Cerrador',
    desc: '5 o más cierres en el período',
  },
  {
    id: 'en_racha',
    emoji: '🔥',
    label: 'En Racha',
    desc: '3 o más cierres en el período',
  },
];
...
export class BiService implements OnModuleInit {
    ...
    const [
      ...
      this.prisma.clientePropiedad.count({
        where: {
          ...
          ...(dateFilter && { fecha_cierre: dateFilter }),
        },
      }),
      this.prisma.clientePropiedad.count({
        where: {
      ...
      this.prisma.visita.count({
        where: {
          ...
          ...(dateFilter && { fecha_inicio: dateFilter }),
        },
      }),
      ...
      this.prisma.brochureDescarga.count({
        where: {
          tenant_id: tenantId,
          ...(dateFilter && { downloaded_at: dateFilter }),
        },
      }),
      ...
      usuarios.map(async (u) => {
        const [
          ...
          this.prisma.clientePropiedad.count({
            where: {
              estado: 'GANADO',
              cliente: { tenant_id: tenantId, agente_id: u.id },
              ...(dateFilter && { fecha_cierre: dateFilter }),
            },
          }),
          this.prisma.clientePropiedad.count({
        ...
        return {
          ...
          activos,
          tasaConversion:
            totalCerrados > 0 ? Math.round((ganados / totalCerrados) * 100) : 0,
  ...
  async getTopPropiedades(
    ...
    if (desde && hasta)
      brochureDateWhere = Prisma.sql`AND bd.downloaded_at >= ${desde} AND bd.downloaded_at <= ${hasta}`;
    else if (desde)
      brochureDateWhere = Prisma.sql`AND bd.downloaded_at >= ${desde}`;
    else if (hasta)
      brochureDateWhere = Prisma.sql`AND bd.downloaded_at <= ${hasta}`;

    const brochureRows = await this.prisma.$queryRaw<
      { propiedad_id: string; total: bigint }[]
    >`
    ...
    const favRows = await this.prisma.$queryRaw<
      { propiedad_id: string; total: bigint }[]
    >`
    ...
    if (desde && hasta)
      emailDateWhere = Prisma.sql`AND ee.abierto_at >= ${desde} AND ee.abierto_at <= ${hasta}`;
    ...
    const emailAperturaRows = await this.prisma.$queryRaw<
      { propiedad_id: string; total: bigint }[]
    >`
    ...
    for (const r of emailAperturaRows)
      emailMap[r.propiedad_id] = Number(r.total);
    ...
    const ranked = props
      .map((p) => {
        ...
        const interacciones = p.interesados.reduce(
          (s, i) => s + i.interacciones.length,
          0,
        );
  ...
  async getRanking(
    ...
    const byComision = [...scored].sort(
      (a, b) => b.comisionTotal - a.comisionTotal,
    );
    const byInteracciones = [...scored].sort(
      (a, b) => b.numInteracciones - a.numInteracciones,
    );
    const byVisitas = [...scored].sort(
      (a, b) => b.visitasRealizadas - a.visitasRealizadas,
    );

    if (byGanados[0]?.ganados > 0) byGanados[0].badges.push('top_ventas');
    if (byComision[0]?.comisionTotal > 0)
      byComision[0].badges.push('top_comision');
    if (byInteracciones[0]?.numInteracciones > 0)
      byInteracciones[0].badges.push('mas_activo');
    if (byVisitas[0]?.visitasRealizadas > 0)
      byVisitas[0].badges.push('tour_master');
  ...
  async getProductividad(
    ...
    if (desde && hasta)
      whereExtra = Prisma.sql`AND i.fecha >= ${desde} AND i.fecha <= ${hasta}`;
    ...
    const totalesTipo: Record<string, number> = Object.fromEntries(
      TIPOS.map((t) => [t, 0]),
    );
  ...
  async getComisiones(tenantId: string, desde?: Date, hasta?: Date) {
    ...
    const realizadas = ganados.reduce(
      (sum, g) =>
        sum + (g.comision_calculada ? Number(g.comision_calculada) : 0),
      0,
    );
    ...
    for (const ep of enProceso) {
      ...
      let monto = 0;
      const tipo = p.gestion;
      if (p.gestion === 'RENTA') {
        // CBR: proyección conservadora = 1 mes de renta (contrato 1-5 años)
        monto = p.precio_renta
          ? Math.round(Number(p.precio_renta) * 100) / 100
          : 0;
      } else {
        ...
        monto = precio
          ? Math.round(Number(precio) * (pct / 100) * 100) / 100
          : 0;
      }
      proyectadas += monto;
      detalleProyectado.push({
        titulo: p.titulo,
        codigo: p.codigo,
        monto,
        estado: ep.estado,
  ...
  async getHeatmap(tenantId: string) {
    ...
    const props = await this.prisma.propiedad.findMany({
      where: {
        ...
        longitud: { not: null },
        estado: {
          in: ['DISPONIBLE', 'RESERVADA', 'EN_NEGOCIACION', 'BORRADOR'],
        },
      },
      select: {
        id: true,
