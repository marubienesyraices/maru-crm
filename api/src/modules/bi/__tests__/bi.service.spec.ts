...
describe('BiService', () => {
  ...
  describe('getResumen', () => {
    ...
    it('calcula tasaConversion e ingresosTotales, y guarda en caché', async () => {
      ...
      prisma.clientePropiedad.aggregate.mockResolvedValue({
        _sum: { comision_calculada: 15000 },
      });
  ...
  describe('getAgentes', () => {
    it('calcula métricas por agente y excluye SUPER_ADMIN', async () => {
      ...
      expect(prisma.user.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ rol: { not: 'SUPER_ADMIN' } }),
        }),
      );
    ...
    it('devuelve caché si existe', async () => {
      mockRedis.get.mockResolvedValue(
        JSON.stringify({ agentes: [{ id: 'cached' }] }),
      );
  ...
  describe('getTopPropiedades', () => {
    it('calcula el score ponderado y ordena de mayor a menor', async () => {
      ...
      expect(result.propiedades[0].score).toBeGreaterThan(
        result.propiedades[1].score,
      );
    ...
    it('respeta el límite solicitado', async () => {
      ...
      const result: any = await service.getTopPropiedades(
        TENANT_ID,
        undefined,
        undefined,
        2,
    ...
    const setupAgentes = (agentes: any[]) => {
      ...
      for (const a of agentes) {
        prisma.clientePropiedad.count
          ...
          .mockResolvedValueOnce(0);
        prisma.clientePropiedad.aggregate.mockResolvedValueOnce({
          _sum: { comision_calculada: a.comision ?? 0 },
        });
  ...
  describe('getProductividad', () => {
    it('agrupa interacciones por tipo y calcula totales', async () => {
      ...
      expect(result.agentes[0].tendencia).toEqual([
        { fecha: '2026-01-01', total: 3 },
      ]);
    });
  });
    ...
    it('suma comisiones realizadas de negocios GANADO', async () => {
      prisma.clientePropiedad.findMany
        .mockResolvedValueOnce([
          {
            comision_calculada: 5000,
            precio_cierre: 100000,
            fecha_cierre: new Date(),
            cliente: { nombre: 'X' },
            propiedad: { titulo: 'A', codigo: 'C1' },
          },
          {
