...
export class VisitasController {
  ...
  @Post()
  @ApiOperation({ summary: 'Agendar nueva visita (envía email al cliente con enlace de gestión)' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar fecha, ubicación o estado de una visita' })
  update(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateVisitaDto) {
  ...
  @Patch(':id/reporte')
  @ApiOperation({ summary: 'Registrar reporte post-visita (marca como REALIZADA)' })
  submitReporte(@Req() req: any, @Param('id') id: string, @Body() dto: ReporteVisitaDto) {
  ...
  async downloadIcs(@Req() req: any, @Param('id') id: string, @Res() res: Response) {
    const ics = await this.service.generateIcs(req.user.tenantId, id);
    res.setHeader('Content-Type', 'text/calendar; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="visita-${id}.ics"`);
    res.send(ics);
  }
}
