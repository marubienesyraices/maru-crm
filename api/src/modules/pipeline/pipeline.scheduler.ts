...
const ACTIVE_STATES = ['NUEVO', 'CONTACTADO', 'INTERESADO', 'EN_NEGOCIACION', 'CIERRE'];
...
export class PipelineScheduler {
  ...
  async checkLeadInactivity() {
    ...
    for (const c of configs) thresholdMap[c.tenant_id] = c.dias_inactividad_lead;
    ...
    if (sent > 0) {
      this.logger.warn(`🔔 Lead inactivity: ${sent} notificación(es) enviada(s)`);
    }
  }
  ...
  async checkNegociacionTimeout() {
    const now = new Date();
    const cutoff = new Date(now.getTime() - NEGOCIACION_TIMEOUT_DAYS * 86_400_000);
    const dedupCutoff = new Date(now.getTime() - DEDUP_WINDOW_DAYS * 86_400_000);
    ...
    const entries = await this.prisma.clientePropiedad.findMany({
      ...
      select: {
        ...
        estado: true,
        updated_at: true,
        cliente: { select: { id: true, nombre: true, agente_id: true, tenant_id: true } },
    ...
    const admins = await this.prisma.user.findMany({
      where: { tenant_id: { in: tenantIds }, rol: { in: ['ADMIN', 'SUPER_ADMIN'] as any[] } },
    ...
    for (const entry of entries) {
      ...
      const days = Math.floor((now.getTime() - entry.updated_at.getTime()) / 86_400_000);
      ...
      for (const adminId of adminsByTenant[entry.cliente.tenant_id] ?? []) recipients.add(adminId);
    ...
    if (sent > 0) {
      this.logger.warn(`⏰ Negociación timeout: ${sent} trámite(s) notificado(s)`);
    }
  }
}
