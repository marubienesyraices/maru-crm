...
function drawImg(doc: any, imgBuffer: Buffer | null, x: number, y: number, w: number, h: number): boolean {
  ...
  try {
    ...
    doc.image(imgBuffer, x, y, { cover: [w, h], align: 'center', valign: 'center' });
    doc.restore();
    return true;
  } catch {
    doc.restore();
...
function drawHeader(
  doc: any,
  opts: {
    W: number; MARGIN: number; primary: string; primaryDark: string; onPrimary: string;
    logoBuf: Buffer | null; tenantNombre: string;
    line1: string; line2?: string; tagline?: string;
    badgeText?: string;
    headerH: number;
  },
): number {
  const { W, MARGIN, primary, primaryDark, onPrimary, logoBuf, tenantNombre,
          line1, line2, tagline, badgeText, headerH } = opts;
  ...
  if (logoDrawW === 0) {
    ...
    doc.fillColor(onPrimary).font('Helvetica-Bold').fontSize(18)
      .text(tenantNombre, textX, 16, { width: textMaxW, lineBreak: false });
  }

  doc.fillColor(onPrimary).font('Helvetica-Bold').fontSize(10)
    .text(line1, textX, logoDrawW > 0 ? 14 : 40, { width: textMaxW, lineBreak: false });

  if (line2) {
    doc.fillColor(onPrimary).font('Helvetica').fontSize(8.5).opacity(0.8)
      .text(line2, textX, doc.y + 3, { width: textMaxW, lineBreak: false })
      .opacity(1);
  }

  if (tagline) {
    doc.fillColor(onPrimary).font('Helvetica').fontSize(7.5).opacity(0.65)
  ...
  if (badgeText) {
    ...
    doc.fillColor(onPrimary).font('Helvetica-Bold').fontSize(7.5)
      .text(badgeText, badgeX, 21, { width: badgeW, align: 'center', lineBreak: false });
  }
...
export class BrochureService {
  ...
  async generateBuffer(propiedadId: string, tenantId: string): Promise<{ buffer: Buffer; codigo: string }> {
    const [propiedad, configIntegraciones, configBrochureRow] = await Promise.all([
      ...
      this.prisma.configBrochure.findUnique({ where: { tenant_id: tenantId } }),
    ]);

    // Resolve section config (sorted by order, using defaults when not configured)
    const rawSecciones = Array.isArray(configBrochureRow?.secciones) && (configBrochureRow!.secciones as unknown as BrochureSeccion[]).length > 0
      ? (configBrochureRow!.secciones as unknown as BrochureSeccion[])
      : DEFAULT_BROCHURE_SECTIONS;
    const secciones = rawSecciones.slice().sort((a, b) => a.order - b.order);
    const seccionVisible = (id: string) => secciones.find(s => s.id === id)?.visible !== false;
    const seccionLabel = (id: string, fallback: string) => secciones.find(s => s.id === id)?.label ?? fallback;
    ...
    const seccionesIzq = secciones
      .filter(s => SECCION_COLUMNA_IZQUIERDA.has(s.id) && s.visible);
    // Right-column sections in configured order
    const seccionesDer = secciones
      .filter(s => SECCION_COLUMNA_DERECHA.has(s.id) && s.visible);
    ...
    drawHeader(doc, {
      W, MARGIN, primary, primaryDark, onPrimary,
    ...
    for (const sec of seccionesIzq) {
      if (sec.id === 'descripcion' && propiedad.descripcion) {
        doc.fillColor('#374151').font('Helvetica-Bold').fontSize(9.5)
          .text(seccionLabel('descripcion', 'DESCRIPCIÓN').toUpperCase(), MARGIN, yLeft);
        yLeft = doc.y + 4;
      ...
      if (sec.id === 'caracteristicas') {
        ...
        if (features.length) {
          doc.fillColor('#374151').font('Helvetica-Bold').fontSize(9.5)
            .text(seccionLabel('caracteristicas', 'CARACTERÍSTICAS').toUpperCase(), MARGIN, yLeft);
          yLeft += 14;
      ...
      if (sec.id === 'amenidades' && Array.isArray(propiedad.amenidades) && (propiedad.amenidades as string[]).length > 0) {
        doc.fillColor('#374151').font('Helvetica-Bold').fontSize(9.5)
          .text(seccionLabel('amenidades', 'AMENIDADES').toUpperCase(), MARGIN, yLeft);
        yLeft += 12;
        doc.fillColor('#4b5563').font('Helvetica').fontSize(9)
    ...
    for (const sec of seccionesDer) {
      if (sec.id === 'ubicacion' && (locationParts || propiedad.direccion)) {
        ...
        doc.fillColor(primary).font('Helvetica-Bold').fontSize(8.5)
          .text(seccionLabel('ubicacion', 'UBICACIÓN').toUpperCase(), COL_RIGHT_X + 10, yRight + 8);
      ...
      if (sec.id === 'agente' && propiedad.agente) {
        ...
        doc.fillColor(primary).font('Helvetica-Bold').fontSize(8.5)
          .text(seccionLabel('agente', 'AGENTE A CARGO').toUpperCase(), COL_RIGHT_X + 10, yRight + 8);
    ...
    if (galleryRow.length > 0 && seccionVisible('galeria_strip')) {
      ...
      if (yContent + 12 < GALLERY_Y) {
        // Section label
        doc.fillColor('#374151').font('Helvetica-Bold').fontSize(9.5)
          .text(seccionLabel('galeria_strip', 'GALERÍA').toUpperCase(), MARGIN, GALLERY_Y);
    ...
    // ─── Footer ───────────────────────────────────────────────────
    const footerLeft = footerTexto ?? `${propiedad.tenant.nombre}  ·  Código ${propiedad.codigo}  ·  ${fecha}`;
    ...
    doc.fillColor(onPrimary).opacity(0.9).font('Helvetica').fontSize(7.5)
      .text(footerLeft, MARGIN, FOOTER_Y + 10, { width: CONTENT_W - 60, align: 'left', lineBreak: false });
    doc.font('Helvetica-Bold')
      .text(watermarkTexto, W - MARGIN - 80, FOOTER_Y + 10, { width: 80, align: 'right', lineBreak: false });
    doc.opacity(1);
    ...
    if (page2Imgs.length > 0 && seccionVisible('galeria_pagina2')) {
      ...
      doc.moveTo(MARGIN, 98).lineTo(W - MARGIN, 98).strokeColor(primary).lineWidth(1).stroke();
