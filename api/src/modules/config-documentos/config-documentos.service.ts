...
} from './brochure-sections.default';
import { CARTA_TEMPLATE_DEFAULT } from './carta-template.default';

const DEFAULT_SECCIONES_JSON =
  DEFAULT_BROCHURE_SECTIONS as unknown as Prisma.InputJsonValue;
...
export class ConfigDocumentosService {
  ...
  async getCartaPlantilla(
    tenantId: string,
  ): Promise<{ plantilla_html: string; es_default: boolean }> {
  ...
  async getBrochureConfig(tenantId: string) {
    const row = await this.prisma.configBrochure.findUnique({
      where: { tenant_id: tenantId },
    });
    if (!row) {
      return {
  ...
  async updateBrochureConfig(tenantId: string, dto: UpdateBrochureConfigDto) {
    ...
    if (dto.secciones !== undefined) data.secciones = dto.secciones;
    if (dto.footer_texto !== undefined)
      data.footer_texto = dto.footer_texto || null;
    if (dto.watermark_texto !== undefined)
      data.watermark_texto = dto.watermark_texto || null;
  ...
  async resetBrochureConfig(tenantId: string) {
    await this.prisma.configBrochure.upsert({
      ...
      update: {
        secciones: DEFAULT_SECCIONES_JSON,
        footer_texto: null,
        watermark_texto: null,
      },
    });
  ...
  parseSecciones(raw: unknown): BrochureSeccion[] {
    if (!Array.isArray(raw) || raw.length === 0)
      return DEFAULT_BROCHURE_SECTIONS;
    return (raw as unknown as BrochureSeccion[]).sort(
      (a, b) => a.order - b.order,
    );
  }
