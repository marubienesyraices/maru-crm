...
export class UsersController {
  ...
  @Get('me')
  @ApiOperation({
    summary: 'Perfil del usuario autenticado (incluye preferencia de tema)',
  })
  findMe(
  ...
  @Patch('me/tema')
  @ApiOperation({
    summary: 'Actualizar preferencia de tema del usuario autenticado',
  })
  updateTema(
    @CurrentUser('sub') userId: string,
    @CurrentUser('tenantId') tenantId: string,
  ...
  @ApiOperation({
    summary:
      'Listar usuarios asignables como agente de una propiedad (SENIOR y ADMIN según plan)',
  })
  ...
  @ApiOperation({
    summary: 'Árbol jerárquico de usuarios (supervisores → subordinados)',
  })
  ...
  @Roles('SUPER_ADMIN')
  @ApiOperation({
    summary: 'Listar todos los administradores de empresas (Solo Super Admin)',
  })
  findAllAdmins() {
  ...
  @Roles('SUPER_ADMIN')
  @ApiOperation({
    summary: 'Crear administrador para una empresa (Solo Super Admin)',
  })
  ...
  @Roles('SUPER_ADMIN')
  @ApiOperation({
    summary: 'Actualizar administrador de empresa (Solo Super Admin)',
  })
  updateAdmin(
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Reenviar correo de activación a usuario pendiente',
  })
  resendActivation(
    @CurrentUser('tenantId') tenantId: string,
  ...
  @ApiOperation({
    summary:
      'Desactivar y resetear el 2FA de un usuario (Admin puede forzar re-configuración)',
  })
  resetTotp(
    @CurrentUser('tenantId') tenantId: string,
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Reasignar todos los subordinados de un Senior a otro supervisor',
  })
  reasignarSubordinados(
    ...
    return this.usersService.reasignarSubordinados(
      tenantId,
      id,
      toSupervisorId,
    );
  ...
  @ApiOperation({
    summary:
      'Desbloquear cuenta de usuario bloqueada por demasiados intentos fallidos',
  })
  desbloquear(
    @CurrentUser('tenantId') tenantId: string,
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @ApiOperation({
    summary:
      'Transferir propiedades y trámites a otro agente y desactivar usuario',
  })
