...
export class ConfigSistemaService {
  private cache: { resend_api_key: string | null; email_from: string } | null =
    null;
  ...
  async findOrCreate() {
    let row = await this.prisma.configSistema.findUnique({
      where: { id: 'singleton' },
    });
    if (!row) {
      row = await this.prisma.configSistema.create({
        data: { id: 'singleton' },
      });
    }
  ...
  async update(dto: UpdateConfigSistemaDto, updatedBy: string) {
    ...
    if (dto.resend_api_key !== undefined) {
      data.resend_api_key = dto.resend_api_key
        ? this.enc.encrypt(dto.resend_api_key)
        : null;
    }
  ...
  async getSystemCredentials(): Promise<{
