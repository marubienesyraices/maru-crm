# Plan de Configuración de Servicios Externos
...
5. Ejecutar migraciones y RLS:
   ...
   ```
6. **Importante:** cambiar la contraseña del rol `maru_app` tras la primera aplicación del script:
   ```sql
   ALTER ROLE maru_app PASSWORD 'contraseña-segura-nueva';
   ```
...
4. Configurar en `.env`:
   ...
   EMAIL_FROM="Maru Bienes y Raíces <noreply@tudominio.com>"
   ```
...
2. Ir a **R2 Object Storage** → **Create bucket** → elegir nombre (ej: `maru-crm-files`).
